...
namespace YazlabDeneme
{
    class KisiBilAgac
    {
        ...
        private void aroundTree(KisiBilNode node)
        {

            if (node != null)
            {
                aroundTree(node.left);
                j++;
                aroundTree(node.right);
            }
        }//ağaçta dolaşıyo
        public void DeleteFile()
        {
            File.Delete(file);
            Write(root);
        }//text dosyasını siliyo güncelliyo
        public void lisansFiltre(KisiBilNode node)
        {
            if (node != null)
            {
                lisansFiltre(node.left);

                if (node.egitimBilListe.count()!=0)
                {
                    filterList.Add(node);
                ...
                lisansFiltre(node.right);
            }
        }//şuanlık bu da hatalı
        public void lisansFiltreOkut()
        {
            lisansFiltre(root);
        }
        public void birdenFazlaDil(KisiBilNode node)
        {
            if (node != null)
            {
                birdenFazlaDil(node.left);

                if (node.YabanciDil.Contains("/"))
                {
                    filterList.Add(node);
                ...
                birdenFazlaDil(node.right);
            }

        }//bu tamam burda aradaki /'a göre birden fazla dil bildiğini gösteriyo
        public void birdenFazlaDilOkut()
        {
            birdenFazlaDil(root);
        }
        public void deneyimsizlerFiltre(KisiBilNode node)//burda hata var düzelt
        {
            if (node != null)
            {
                deneyimsizlerFiltre(node.left);

                if (node.isYeriList.count() == 0)
                {
                    filterList.Add(node);
                ...
                deneyimsizlerFiltre(node.right);
            }
        }
        public void deneyimsizlerFiltreOkut()
        {
            deneyimsizlerFiltre(root);
        }
        public void ismeGoreFiltre(KisiBilNode node,string arananAd)
        {
            if (node != null)
            {
                ismeGoreFiltre(node.left,arananAd);

                if (node.Ad.Contains(arananAd))
                {
                    filterList.Add(node);
                ...
                ismeGoreFiltre(node.right, arananAd);
            }
        }//Girilen isme göre fitreleme işlemi tamam
        public void ismeGoreFiltreOkut(string arananAd)
        {
            ismeGoreFiltre(root, arananAd);
        }
        public void DosyayiSil()
        {
            File.Delete(file);
        }//text dosyasını siliyo
    }
}
